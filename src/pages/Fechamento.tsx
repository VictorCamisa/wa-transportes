...
const Fechamento = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const { data, error } = await (supabase
        ...
        .lte('data_servico', formatDateForDatabase(endDate))
  ...
  const generatePDF = () => {
    ...
    // Calcular total
    const total = services.reduce((acc, service) => acc + (service.valor_numerico || 0), 0);
  ...
  const totalValue = services.reduce((acc, service) => acc + (service.valor_numerico || 0), 0);

  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center space-x-2">
        <Calendar className="h-6 w-6" />
        <h1 className="text-2xl font-bold">Fechamento de Serviços</h1>
      </div>
